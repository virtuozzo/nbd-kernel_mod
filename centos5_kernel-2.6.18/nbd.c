...
void nbd_resp_timeout(unsigned long arg)
{
	...
	if ( lo->client_task ) {
		...
		printk(KERN_ALERT "%s: Server not responding after %d seconds - killing client (pid:%d %s)\n",
							lo->disk->disk_name, lo->xmit_timeout/HZ, lo->client_pid, lo->conn_info.cidata );
		dump_last_requests(lo);
		...
		if ( lo->client_task ) {
			...
			printk(KERN_ALERT "%s: Too slow to kill, client is already dead (pid:%d)\n",
							lo->disk->disk_name, lo->client_pid );
		}
		...
		int msg_flags)
{
	...
	do {
		...
		if (signal_pending(current)) {
			siginfo_t info;
			spin_lock_irqsave(&current->sighand->siglock, flags);
			printk(KERN_ALERT "%s: (pid %d: %s) got signal %d\n",
				lo->disk->disk_name, current->pid, current->comm, 
...
static int nbd_send_req(struct nbd_device *lo, struct request *req)
{
	...
	} else if ( req->flags == REQ_SPECIAL && nbd_cmd(req) == NBD_CMD_SRVCMD ) {
...
static struct request *nbd_read_stat(struct nbd_device *lo)
{
	...
	dprintk(DBG_QHASH|DBG_IOCMDS, "%s RESP: Received nbd_reply: %lu bytes\n", lo->disk->disk_name, sizeof(struct nbd_reply));
	...
	if ( atomic_read(&lo->srvcmd_pending) > 0 ) { /* pending srvcmd reqs? */
		...
		dprintk(DBG_IOCMDS, "%s RESP: Checking for Server CMD reply! (%d pending reqs, creq= %p)\n",
	   	...
	   	if ( creq->flags == REQ_SPECIAL && nbd_cmd(creq) == NBD_CMD_SRVCMD) {
		...
		else if ( creq->flags == REQ_SPECIAL && nbd_cmd(creq) == NBD_CMD_FLUSH) {
...
static void nbd_do_it(struct nbd_device *lo)
{
	...
	if (lo->active_req != NULL || !list_empty(&lo->queue_head) || 
		atomic_read(&lo->qhash_pending) || atomic_read(&lo->srvcmd_pending) )
		printk(KERN_ALERT "%s INIT: WARNING! active_req: %p, qhash_pending: %d, srvcmd_pending: %d, EMPTY queue_head= %d\n",
	...
	if ( atomic_read( &lo->qhash_pending ) != 0 ) {
		...
		if ( lo->qhash_req ) {
			lo->qhash_req->errors++;
			complete( (struct completion *)lo->qhash_req->waiting );
		}
		if ( atomic_read( &lo->qhash_pending ) > 0 )
			atomic_dec( &lo->qhash_pending );
	...
	if ( atomic_read( &lo->srvcmd_pending ) != 0 ) {
		...
		if ( lo->srvcmd_req ) {
			...
			complete( (struct completion *)lo->srvcmd_req->waiting );
		}
		if ( atomic_read( &lo->srvcmd_pending ) > 0 )
			atomic_dec( &lo->srvcmd_pending );
	...
	while (!list_empty(&lo->queue_head)) {
		...
		if ( req->flags == REQ_SPECIAL ) {
			if ( nbd_cmd(req) == NBD_CMD_QHASH ) { /* qhash req? */
				complete( (struct completion *)req->waiting );
				atomic_dec( &lo->qhash_pending );
				continue;
...
#ifdef ENABLE_REQ_DEBUG
	printk(KERN_WARNING "%s: queue cleared [pendRQ:%d/%d R:%d/%d W:%d/%d]\n",
...
qcompl_wait:
			err = wait_for_completion_interruptible( (struct completion *)qreq->waiting);
			if (unlikely(err)) {
				if (err == -ERESTARTSYS)
					goto qcompl_wait;
...
cinfo_exit:
	...
	case NBD_SERVER_CMD:
	{
		...
		if (!lo->file || !lo->sock) { /* NOT CONNECTED */
			...
			/* _____________________________________
			 * setup the command request header... */
			creq->flags = REQ_SPECIAL;
			nbd_cmd(creq) = NBD_CMD_SRVCMD;

			/* NOTE: assuming a sector size= 512 bytes! */
			creq->sector = 0;
			creq->hard_nr_sectors = 8; /* == blocksize */
			creq->nr_sectors = 8; /* == blkcount */
			...
			dprintk( DBG_IOCMDS, "------------\nSENDING SERVER_CMD REQ: sector= %llu, nr_sectors=%lu\n", creq->sector, creq->nr_sectors );
			INIT_COMPLETION(lo->srvcmd_wait);
			creq->waiting = &lo->srvcmd_wait;
...
scompl_wait:
				err = wait_for_completion_interruptible( (struct completion *)creq->waiting);
				if (unlikely(err)) {
					if (err == -ERESTARTSYS)
						goto scompl_wait;
...
srvcmd_exit:
	...
	case NBD_CLEAR_SOCK:
		lo->harderror = 0;
		//printk(KERN_ALERT "%s: NBD_CLEAR_SOCK call\n", lo->disk->disk_name );
	...
	case NBD_DO_IT:
		 ...
		 * there should be a more generic interface rather than
		 * calling socket ops directly here */
		//printk(KERN_ERR "%s: EXITING thread...\n", lo->disk->disk_name );
shutdown_socket:
		sock_shutdown(lo, 1);
